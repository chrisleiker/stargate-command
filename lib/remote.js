 ...
 * as an option, so an unchecked host could quietly turn into a flag. macOS
 * has the same problem in a different shape — the host is pasted into an
 * rdp:// URL, where a stray separator would add settings rather than a flag.
 * Only a hostname, an IPv4 address, or a bracketed IPv6 address gets through
 * here, with an optional port and nothing else.
 *
...
/*
 * macOS has no RDP binary to look for. Both Windows App and the Microsoft
 * Remote Desktop it replaced register the rdp:// scheme, so the request goes
 * to Launch Services and whichever of them is installed answers it.
 *
 * The key really is spelled "full address", space and all, so it has to be
 * percent-encoded; so do the brackets of an IPv6 literal. parseHost has
 * already ruled out every other character that would mean something here.
 */
function macRemoteUrl(spec) {
  const address = String(spec || '')
    .replace(/\[/g, '%5B')
    .replace(/\]/g, '%5D');
  return 'rdp://full%20address=s:' + address;
}

module.exports = { parseHost, LINUX_CLIENTS, macRemoteUrl };
